		...
		namespace detail
		{
			...
			public:
				...
				*/
				...
				// 由给定的size和values来构建lut, (这里默认channel为3)
				// values为lut中按顺序读的每一个数
				void create(std::uint32_t size,  std::vector<float> values) noexcept(false)
				{
					if (values.size() == size * size * size * 3)
					{
						this->width = size * size;
					...
					{
						throw parse_error::create(0, 0, "The lut element does not match the size of vector<float> values.");
					}					
				}
